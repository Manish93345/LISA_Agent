"""
LISA — Main Agent (with Action System)
"""
...
class LisaAgent:
    ...
    def chat(self, user_message: str) -> str:
        ...
        if action_result is not None:
            ...
            # Action hua — ab LISA conversationally respond kare
            # Action ka result + user message dono context mein dena
            system_prompt = self._build_system_prompt(user_message)

            if success:
                # Tell LISA: action ho gaya, ab naturally confirm karo
                augmented_msg = (
                    f"{user_message}\n\n"
                    f"[System: Action successfully executed — {action_msg}. "
                    f"Ab in words mein naturally confirm karo, "
                    f"same tone mein jaise hum baat kar rahe hain. "
                    f"Short response dena.]"
                )
            else:
                augmented_msg = (
                    f"{user_message}\n\n"
                    f"[System: Action failed — {action_msg}. "
                    f"Naturally apologize karo aur batao kya problem aayi.]"
                )
